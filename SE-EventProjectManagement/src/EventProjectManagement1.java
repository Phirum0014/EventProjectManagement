...
public class EventProjectManagement1 {
...
private static void createEvent() {
    ...
    private static void viewTickets() {
        ...
        boolean isOwner = tickets.stream()
            .anyMatch(ticket -> ticket.eventName.equals(event.name) && ticket.username.equals(loggedInUser));
        ...
        if (userRoles.get(loggedInUser).equals("admin")) {
            ...
            System.out.print("Do you want to delete the user account or just the tickets? (a/t): ");
            ...
            tickets.removeIf(ticket -> ticket.username.equals(usernameToDelete));
            saveTickets();
            System.out.println("All tickets for user " + usernameToDelete + " have been deleted.");
            } else {
            System.out.println("Invalid choice. No action taken.");
            }
        } else {
            System.out.println("You do not have permission to delete other users.");
            System.out.print("Do you want to delete your own account or just your tickets? (a/t): ");
            String choice = scanner.nextLine().toLowerCase();
            ...
            System.out.print("Do you want to delete a specific ticket or all tickets? (s/a): ");
            String ticketChoice = scanner.nextLine().toLowerCase();

            if (ticketChoice.equals("s")) {
            System.out.print("Enter the ticket number to delete: ");
            String ticketNumberInput = scanner.nextLine();

            if (!ticketNumberInput.matches("\\d+")) {
                System.out.println("Invalid input. Please enter a number.");
                return;
            }

            int ticketNumber = Integer.parseInt(ticketNumberInput);
            boolean ticketFound = tickets.removeIf(ticket -> ticket.username.equals(loggedInUser) && ticket.ticketNumber == ticketNumber);
            ...
            } else if (ticketChoice.equals("a")) {
            tickets.removeIf(ticket -> ticket.username.equals(loggedInUser));
            saveTickets();
            System.out.println("All your tickets have been deleted.");
            } else {
            System.out.println("Invalid choice. No action taken.");
            }
            } else {
